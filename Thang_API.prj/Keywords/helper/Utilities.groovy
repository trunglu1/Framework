...
public class Utilities {
	...
	def getDataRow(String dataFile, int rowIndex) {
		...
		try{
			String[] headerNames = data.getColumnNames()
			for (header in headerNames) {
				_dataDict[header] = data.getValue(header, rowIndex)
			}
		} catch (Exception ex){
	...
	def verifyCountMatchs(String str, String findStr, int expectedNumber){
		...
		WebUI.verifyEqual(findText, expectedNumber, FailureHandling.STOP_ON_FAILURE)
	}
	...
	def verifyListValues(def listValues1, def listValues2, String operator='='){
		if (operator == '='){
			...
			WebUI.verifyEqual(_list1, _list2, FailureHandling.STOP_ON_FAILURE)
		} else{
			listValues2.each{itemValue ->
				if(!listValues1.contains(itemValue))WebUI.verifyEqual(listValues1.toString(), itemValue.toString(), FailureHandling.STOP_ON_FAILURE)
			}
		}
	}
